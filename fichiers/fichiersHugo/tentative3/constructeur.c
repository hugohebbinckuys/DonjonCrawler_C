...
{
    salle salle_vide; 
    initialiser_salle (&salle_vide, 2, 2, 1, 1);
    printf ("debug salle_vide : \n");
    afficher_salle (&salle_vide); 
    printf("debug salle_vide ok\n");
    int nb_salles; 
    printf ("Combien pensez-vous inserer de salles au maximum ?\n"); 
    scanf("%d", &nb_salles); 
    sto->tab_stockage = (salle **)malloc(sizeof(salle *) *(nb_salles+10)); // on prévoit jusqu'à 10 salles au dessus de ce qu'il avait prévu vraiment au cas ou ùais au moins le malloc est suffisant noramlent. 
    for (int i = 0; i < (nb_salles+10); i++)
    {
        sto->tab_stockage[i] = &salle_vide;
        printf ("debug dans init de salle_vide a l'interieur du stockage : salle %d\n", i);
        afficher_salle(sto->tab_stockage[i]);
        printf ("debug insertion dans stockage ok\n");
        sto->nb_element ++; 
        printf ("nb_element : %d\n", sto->nb_element);
    }
    // sto->nb_element = nb_salles;
    // printf ("init du stockage ok\n");
}
...
void initialiser_donjon(donjon *d, int h, int l)
{
    ...
    d->stockage = &sto;
    printf ("debugage du stockage du donjon : nb_element : %d\n", d->stockage->nb_element); // le debug est bon, on est toujours à 10 salles 
    ...
    printf ("2e debugage du stockage du donjon : nb_element : %d\n", d->stockage->nb_element);
    for (int i = 0; i < d->hauteur_dj; i++) // on parcours le tableau de pointeur
    {
        ...
        for (int j = 0; j < d->largeur_dj; j++) // parcours le tableau de caractère
        {
            ...
            {
                // printf ("3e debugage du stockage du donjon : nb_element : %d\n", d->stockage->nb_element); // ok 
                d->tab_dj[i][j] = '#';
            }
            else
            {
    ...
    printf ("4e debugage du stockage du donjon : nb_element : %d\n", d->stockage->nb_element);
}
...
{
    printf ("5e debugage du stockage du donjon : nb_element : %d\n", dans->stockage->nb_element);
    printf ("6e debugage du stockage du donjon : nb_element : %d\n", dans->stockage->nb_element);

    int i=5;
    printf ("11e debugage du stockage du donjon : nb_element : %d\n", dans->stockage->nb_element);
    int j;
    printf ("12e debugage du stockage du donjon : nb_element : %d\n", dans->stockage->nb_element);
    int cpt_x = 4;
    printf ("13e debugage du stockage du donjon : nb_element : %d\n", dans->stockage->nb_element);
    int cpt_y = 0 ;
    printf ("7e debugage du stockage du donjon : nb_element : %d\n", dans->stockage->nb_element);
    int y = s->y;
    printf ("8e debugage du stockage du donjon : nb_element : %d\n", dans->stockage->nb_element);
    int x = s->x;
    printf ("9e debugage du stockage du donjon : nb_element : %d\n", dans->stockage->nb_element);
...
int chevaucher(donjon *dans, salle *s)
{
    ...
    {
        printf ("CHEV-SUPPR : ");
        afficher_salle (dans->stockage->tab_stockage[i]);
...
{
    printf ("CHEV-SUPPR : ");
    afficher_salle (d->stockage->tab_stockage[indice_salle]);
