...
{
    printf ("debug est ce que l'initialisation du stockage des salles se fait vraiment ? \n");// oui c ok 
    ...
    sto->nb_element = 0;
    printf ("debug nb_element a l'initialisation du stockage : %d\n", sto->nb_element); // c ok on est a 0
}
...
salle *copier_salle(salle *src){
    salle *nouvelle = (salle *) malloc(sizeof(salle)); 
    nouvelle->hauteur_salle = src->hauteur_salle; 
    nouvelle->largeur_salle = src->largeur_salle; 
    nouvelle->x = src->x; 
    nouvelle->y = src->y; 

    nouvelle->tab_salle = (char**) malloc(sizeof(char*)*src->hauteur_salle); 
    for (int i=0; i<src->hauteur_salle; i++) {
        nouvelle->tab_salle[i] = (char*)malloc(sizeof(char)*(src->largeur_salle+1));
        strcpy(nouvelle->tab_salle[i], src->tab_salle[i]);
    }
    return nouvelle; 
}
...
void inserer_salle(donjon *dans, salle *s)
{
    ...
    int y = s->y;
    int x = s->x;
    printf ("debug1 pas de pb\n");
    for (i = y; i < y + s->hauteur_salle && i < dans->hauteur_dj; i++)
    {
        printf ("debug2 entree dans la boucle1\n");
        for (j = x; j < x + s->largeur_salle && j < dans->largeur_dj; j++)
        {
            printf ("debug3 entree dans la boucle2\n");
            printf ("debug on regarde ce que retourn chevaucher : \n");
            printf (" %d\n", chevaucher(dans, s));
            // break; // on retourn bien -1 donc on doit passer dans le else de la boucle 
            if (chevaucher(dans, s) != -1)
            {
                printf ("debug4 entree dans la deuxieme boucle, normalement marche pas\n");
                // en effet ca marche pas => on rentre pas dans la boucle2 => pb dans chevaucher 
                supprimer_salle(dans, chevaucher(dans, s));
                inserer_salle(dans, s);
            }
            else
            {
                printf ("debug normalement on devrait arriver ici et inscrire la table dans le tableau de stockage OK?\n");
                printf ("debug i-y = %d\n", i-y);
                printf ("debug j-x = %d\n", i-y);
                dans->tab_dj[i][j] = s->tab_salle[i - y][j - x]; // modification
                // c ok on est passé la
            }
        }
    }
    printf ("debug nb_element avant modif :%d\n", dans->stockage->nb_element);
    // on s'arrete ici => rien ne se passe ici 
    printf (" debug on va afficher la salle qu'on veut ajouter dans le tableau pour voir si tvb : \n"); // ca c ok
    afficher_salle(s);
    // ligne du dessous pose probleme //
    // dans->stockage->tab_stockage[dans->stockage->nb_element] = s; // c ici le pb enft tout simplement  

    //ajouts//
    salle *copie_s = (salle*)malloc(sizeof(salle));
    copie_s = copier_salle(s); 
    dans->stockage->tab_stockage[dans->stockage->nb_element] = copie_s; 
    printf("debug est c ok ??\n");
    //fin ajouts // 
    
    printf("pb?\n");
    dans->stockage->nb_element += 1;
    printf ("debug nb_element apres modif :%d\n", dans->stockage->nb_element);
    printf ("debug SI AUCUN PROBLEME DEVRAIT PRINTF CA STP \n");
}
// ccl le bug provient du fait que le nb_element du stockage est un chiffre aberrant => fait tout bug car pas mal de choses reposent sur ce chiffre 
...
int chevaucher(donjon *dans, salle *s)
{
    ...
    int i;
    printf ("nb_element dans stockage des salles du donjon : %d\n", dans->stockage->nb_element);
    for (i = 0; i < dans->stockage->nb_element; i++)
    {
        printf ("debug CH2\n"); // => on ne rentre pas dans cette boucle, pourquoi ? 
    ...
    printf ("est ce que on passe par ici ?\n");
    return -1;
}
