...
void initialiser_salle (salle *s) {
    ...
    for (int k = 0; k<elts; k++) {
        // appel de la fonction inserer dans salle 
        inserer_dans_salle (s);
    }
}

// void initialisation_plusieurs_salles (){
//     int nb_salles; 
//     printf ("Combien de salle(s) voulez-vous creer ?");
//     scanf("%d", &nb_salles);
//     if (nb_salles>0 && nb_salles == 1){
//         initialiser_salle (); 
//     }
// }
...
//     salle s = initialiser_salle(); 
//     printf("salle initialisé");
//     inserer_salle_dans_donjon (&d, &s);
//     printf ("salle normaalmeent initialisée dans o=donjon");
//     afficher_donjon(&d);
//     liberer_salle(&s);
//     liberer_donjon(&d);
// }
...
int main () {
    ...
    initialiser_donjon(&d);
    int nb_salles; 
    printf ("combien de salles voulez-vous mettre dans le donjon");
    scanf("%d", &nb_salles);
    salle **tab = (salle **)malloc(sizeof(salle *) * nb_salles);
    ...
    for (int j = nb_salles; j>0; j--){
        inserer_salle_dans_donjon(&d, tab[j]);
        // if (chevauche(tab[j])){
        //     printf ("La nouvelle salle chevauche une autre table, elle a donc supprimé l'ancienne table");
        // }
    }
    afficher_donjon (&d);
    liberer_donjon(&d);
