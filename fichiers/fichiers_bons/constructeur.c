...
void initialiser_salle(salle *s, int h, int l, abcisse x, ordonne y)
{
    ...
    // int arret = 0; 
    // while (arret==0){
    //     scanf (" %c", &sauv); //' ' espace important sinon bug 
    //     if (sauv == 'n') {
    //         arret = 1;
    //     }
    //     else if (sauv == 'o') {
...
void inserer_salle(donjon *dans, salle *s)
{
    ...
    if (indiceChevauchante != -1)
    {
        ...
        printf("la salle chevauche une salle existante, suppression de la salle précédent et mise en place de la nouvelle salle...");
...
int chevaucher(donjon *dans, salle *s) {
    int x_new = s->x;
    int y_new = s->y;
    int largeur_new = s->largeur_salle;
    int hauteur_new = s->hauteur_salle;
    ...
    for (i = 0; i < dans->stockage->nb_element; i++) {
        ...
        int x_old = dans->stockage->tab_stockage[i]->x;
        int y_old = dans->stockage->tab_stockage[i]->y;
        int largeur_old = dans->stockage->tab_stockage[i]->largeur_salle;
        int hauteur_old = dans->stockage->tab_stockage[i]->hauteur_salle;

        // Vérifier le chevauchement en termes de coordonnées et de tailles
        if (x_old < x_new + largeur_new &&
            x_old + largeur_old > x_new &&
            y_old < y_new + hauteur_new &&
            y_old + hauteur_old > y_new) {
            return i; // Il y a chevauchement
        }
    }
...
void supprimer_salle(donjon *d, int indice_salle)
{
    ...
    FILE *f;
    f = fopen(salle_a_supprimer->fichier, "w+");
    // fseek(f, 0, SEEK_END); // deplace le cursu=eur a la fin du fichier 
    fprintf(f, " La salle n'est plus dans le donjon car elle a ete remplacee");
    // fflush(f); // assure que les données sont ecrits 
...
void sauvegarder_salle_dans_fichier (salle *s){
    ...
    scanf("%s", nom_fichier); // Utilisation de %29s pour éviter les dépassements de tampon eventuellement 
...
void sauvegarder_donjon_dans_fichier_sans_pointeur (donjon d){
    FILE *f; 
    f=fopen("sauvegarde_donjon.txt", "w");
    fprintf(f, "TEST \n\n"); 
    fflush(f);
...
donjon creation_donjon () {
    int h_dj; 
    printf ("Quelle hauteur voulez-vous donner à votre donjon ?\n");
    scanf("%d", &h_dj);
    int l_dj; 
    printf ("Quelle largeur voulez-vous donner à votre donjon ?\n");
    scanf("%d", &l_dj);
    donjon d; 
...
salle creation_salle() {
    salle s; 
    printf ("Quelle hauteur pour la salle :");
    int h_s;
    scanf("%d", &h_s); 
    printf ("Quelle largeur pour la salle :");
    int l_s; 
    scanf("%d", &l_s);
    ...
    char elt;
    do {
        printf ("Voulez-vous inserer une entitee dans la salle ? ");
        scanf(" %c", &insertion);
        if (insertion == 'o'){
            entitee e;
            printf ("Saisissez l'entitee que vous voulez inserer : ");
            scanf (" %c", &elt);
            abcisse x; 
...
void insertion_salle (donjon *d, salle *s) {
    printf ("voulez-vous inserer la salle dans le donjon ?");
    char sauv; 
    scanf(" %c", &sauv);
    if (sauv == 'o'){
        if (d->hauteur_dj != -10) {
...
void afficher_menu () {
    ...
    printf("i -> Inserer une salle dans le donjon\n");
...
void fonction_finale (){
    printf ("Indiquez si vous voulez creer un donjon (d), creer une salle (s), quitter le concepteur de donjon (q). \n");
    char commande; 
    scanf("%c", &commande);
    while (commande != 'q'){
        if (commande == 'd') {
            printf ("Creation de salle en cours ...\n");
            donjon d = creation_donjon(); 
            afficher_donjon(&d);
    ...
    // printf ("Voulez-vous créer une salle ? 'o' ou 'n'\n");

    // salle s1; 
    // initialiser_salle (&s1, 5, 5, 2, 2);
    // entitee e1;
    // initialiser_entitee (&s1, &e1, 0, 0, 'F');
